...
const detectDevice = {
  ...
  checkWindowSize() {
    ...
    this.resizeTimer = setTimeout(this.checkIsLandscape.bind(this), 250);
  },

  checkIsLandscape(slider=null) {
    this.checkIsThinScreen();

    if (!this.isLandscape() && app.turnDeviceContent.css('display') === 'none') {
      app.turnDeviceContent.fadeIn();
      if (slider) {
        slides.destroySlider();
      }
    } else if (this.isLandscape() && app.turnDeviceContent.css('display') === 'block') {
      app.turnDeviceContent.fadeOut();
  ...
  checkIsThinScreen() {
    ...
    console.log(widthHeightProportion, $(window).width(), $(window).height());
